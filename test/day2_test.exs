defmodule Day2Test do
  ...
  describe "code_list/2" do
    test "returns result" do
      assert Day2.code_list(5, [["U"], ["D"], ["L"], ["R"]]) == [2, 5, 4, 5]
    end

    test "returns result of first movement" do
      assert Day2.code_list(5, [Enum.at(Day2.parse_input(ReadInput.from_day(2)), 0)]) == [3]
    end

    test "returns result of second movement" do
      assert Day2.code_list(3, [Enum.at(Day2.parse_input(ReadInput.from_day(2)), 1)]) == [3]
    end

    test "returns result of 3rd movement" do
      assert Day2.code_list(3, [Enum.at(Day2.parse_input(ReadInput.from_day(2)), 2)]) == [4]
    end

    test "returns result of 4th movement" do
      assert Day2.code_list(4, [Enum.at(Day2.parse_input(ReadInput.from_day(2)), 3)]) == [4]
    end

    test "returns result of 5th movement" do
      assert Day2.code_list(4, [Enum.at(Day2.parse_input(ReadInput.from_day(2)), 4)]) == [4]
    end

    test "returns result of input" do
      assert Day2.code_list(5, Day2.parse_input(ReadInput.from_day(2))) == [3, 3, 4, 4, 4]
    end

    test "example" do
      assert Day2.code_list(5, Day2.parse_input("ULL
RRDDD
LURDL
UUUUD"
      )) == [1, 9, 8, 5]
    ...
    test "direct input" do
      assert Day2.code_list(5, Day2.parse_input("
